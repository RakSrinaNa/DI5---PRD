...
\chapter{Analyse et conception}
	...
	\section{Analyse théorique}
		...
		\subsection{Rayons de rechargement différents}
			...
			Une manière simple de prendre en compte des rayons différents est de poser $M=\min_{i}M_i$.
		...
		\subsection{Modification de la méthode de clustering}
			...
			\begin{algorithm}
				...
				\Input{The set $S=\left\{1,\dots, m\right\}$ of $m$ regions obtained from the 2 firsts steps of the first step. The set $S_j=\left\{1,\dots ,n_j\right\}$ of $n_j$ sensors inside a region $R_j$ with their charging time $t_i$, $\forall i\in S$.}
				%\Output{A partition of the bitmap}
				\BlankLine
				\For{$j\in S$}{
					$Acc\leftarrow \left[\right]$ an array of size $n_j$ initialised with 0s that will represent the accumulated charging time for each sensor.
					
					\For{$i\in S_j\prive{n_j}$}{
