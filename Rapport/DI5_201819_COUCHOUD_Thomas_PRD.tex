	...
	\section{Analyse théorique}
		...
		\subsection{Modification de la méthode de clustering}
			...
			\begin{algorithm}
				...
				\Input{The set $S=\left\{1,\dots, m\right\}$ of $m$ regions obtained from the 2 firsts steps of the first step. The set $S_j=\left\{1,\dots ,n_j\right\}$ of $n_j$ sensors inside a region $R_j$ with their charging time $t_i$, $\forall i\in S$. The set $K$ which for each sensor contains a set containing each regions the sensor is in.}
				%\Output{}
				\BlankLine
				\For{$j\in S$}{
					$P_j\leftarrow \left\{\right\}$
					
					$t_j\leftarrow 0$
					
					\For{$i\in S_j$}{
						\eIf{$\text{card}\left(K_i\right)>1$}{
							$P_j\leftarrow P_j\cup\left\{ i\right\}$
							
							$K_i \leftarrow K_i\prive{j}$
						}{
							$t_j \leftarrow \max\left( t_j, t_{j,i}\right)$
						}
					}
					
					\For{$i\in P_j$}{
						$t_{j,i}\leftarrow t_{j,i} - t_j$
					}
				}
			\end{algorithm}
