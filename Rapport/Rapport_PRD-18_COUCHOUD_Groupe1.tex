...
}{Images/UI1.png}{dictum elit condimentum}

\posterblock{Mise en \oe{}uvre}{
Mauris consectetur tellus ac ligula pharetra, et auctor mi \emph{fermentum}.  Etiam venenatis augue neque, ac ullamcorper risus sodales sit amet.
\begin{center}
...
\end{itemize}
}{Images/UI1.png}{auctor a non nisi}
...
Avancement:
\begin{itemize}
	\item Correction de problèmes où la map n'affichait pas correctement le status de certains éléments.
	\item Correction de NullPointerExceptions lors de la mise à jour des zones en conflict.
...
Avancement:
\begin{itemize}
	\item Arrête ORTools si le temps de calcul est trop long (permet d'éviter les bloquages de ce dernier).
	\item Executions de simulation pour obtenir des premiers résultats.
\end{itemize}
}
...
\begin{itemize}
	...
	\item Amélioration d'ORTools en définissant un espace de recherche réduit.
	\item Ecriture du cahier de tests.
	\item Ecriture de la partie guide d'utilisation.
	\item Completion de la partie guide d'installation.
\end{itemize}
}
...
\chapter{Cahier du développeur}
	...
	\section{Description des classes}
		...
		\subsection{Diagramme de classes\label{sec:classes}}
			\img{DetailledClassDiagram.png}{Diagramme de classes un peu plus détaille}{}
	...
	\section{Structure du projet}
		...
		\dirtree{%
			.1 .. 
			.2 libs\DTcomment{Dossier contenant la dépendance ortools pour maven (repo local)}. 
			...
			.2 native\DTcomment{Dossier contenant les dll/dylib/so pour ortools}. 
			.3 <numéro de version>. 
			.4 windows. 
			.4 osx. 
			.4 linux. 
			...
			.4 resources\DTcomment{Resources pour la production (images pour l'UI, log4j, ...)}. 
			.3 test\DTcomment{Sources de test}. 
			.4 java\DTcomment{Sources java pour les tests}. 
			.4 resources\DTcomment{Resources pour les tests (fichiers de configuration à parser}. 
...
\chapter{Cahier des tests}
	...
	\section{Tests fonctionnels}
		...
		\subsection{Instances des tests fonctionnels}
			\jsonsourcefile[caption=test1.json]{../Simulator/test1.json}
			\jsonsourcefile[caption=test2.json]{../Simulator/test2.json}
			\jsonsourcefile[caption=test3.json]{../Simulator/test3.json}
			\jsonsourcefile[caption=test4.json]{../Simulator/test4.json}
			\jsonsourcefile[caption=test5.json]{../Simulator/test5.json}
			\jsonsourcefile[caption=test6.json]{../Simulator/test6.json}
